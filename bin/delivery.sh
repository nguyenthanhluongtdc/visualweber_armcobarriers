...
BACKUP_PATH_PROJECT="$(date +"%d.%m.%y")_armcobarriers"
...
BACKUP_SOURCENAME=armcobarriers_source-$(date +"%Y-%m-%d_%H.%I.%S").zip
BACKUP_SQLNAME=armcobarriers_dbs-$(date +"%Y-%m-%d_%H.%I.%S").sql
...
## echo -e "$VERT--> http://armcobarriers.dev.gistensal.com/$BACKUP_SOURCENAME $NORMAL"
## echo -e "$VERT--> http:/armcobarriers.dev.gistensal.com/$BACKUP_SQLNAME $NORMAL"

<nav class="navbar navbar-expand-lg navbar_mobie">
    ...
    <a href="{{route('public.index')}}" class="logo__link" title="Logo">
    ...
    <div class="toolbar-mobile">
      <ul class="list-tool">
          ...
          <li>   
            <a href="{{route('public.cart')}}" title="Cart">
          ...
          <li>
            <ul class="header_list">
                ...
                @else
                    <li class="img-user">
                        <a href="{{ route('customer.overview') }}" title="Overview">
          ...
          <li>
            <a href="{{theme_option('footer-social')}}" title="Facebook">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      ...
      <ul class="navbar-nav mr-auto mt-2 mt-lg-0">
        ...
        <li class="nav-link nav-item dropdown dmenu">
            ...
            <a id="nav-bar-dropdown" class=" nav__menu-link dropdown-toggle" href="{{$row->url }}" title="{{$row->name}}">
                {{ $row->name }}
            </a>
            <div class="dropdown-menu sm-menu menu-pc-dropdown">
                                ...
                                <ul class="list-unstyled">
                                    <li class="">
                                        <div class="category-type">
                                            <a href="{{ $child->url }}" data-toggle="{{$child->has_child ? 'collapse' : ''}}  " aria-expanded="true"  title="{{$child->name}}" >{{ $child->name }}</a>
                                            @if ($child->has_child)
                                    ...
                                    <div class="">
                                        @if ($child->has_child)
                                            @foreach($child->child as $key => $grandChild)
                                                <li>
                                                    <a href="{{$grandChild->url}}" title="{{$grandChild->name}}">{{$grandChild->name}}</a>
                                                </li>
                                            @endforeach
                                        @endif
                                    </div> 
                ...
                <div class="menu-mb" style="display: none">
                    <a href="{{ $child->url }}" class="dropdown-toggle caret" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false" title="{{$child->name}}">{{ $child->name }}</a>
        ...
        <li class="nav-link nav-item @if ($row->url == Request::url()) active current  @endif">
            <a href="{{$row->url}}" class="nav__menu-link" target="{{ $row->target }}" title="{{$row->name}}">
